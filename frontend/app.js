...
const state = {
  ...
  chats:        [],           // полный массив чатов
  activeChatId: null,
  onlineUsers:  new Set(),
  ws:           null,
  wsHeartbeat:  null,         // interval id
...
async function loadChats() {
  ...
  state.chats = res.data.map(chat => ({
    ...
    lastMsgTime:    chat.messages?.[0]?.createdAt || chat.updatedAt,
    lastSenderNick: chat.messages?.[0]?.sender?.nickname || '',
    _typing:        false,
  }));
